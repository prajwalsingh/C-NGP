...
__global__ void raymarching_train_kernel(
    ...
    torch::PackedTensorAccessor64<int64_t, 1, torch::RestrictPtrTraits> rays_index
){
...
std::vector<torch::Tensor> raymarching_train_cu(
    ...
    auto rays_index = torch::zeros({N_rays*max_samples, 1},
                        torch::dtype(torch::kLong).device(rays_o.device()));
    ...
    ([&] {
        raymarching_train_kernel<<<blocks, threads>>>(
            ...
            rays_index.packed_accessor64<int64_t, 1, torch::RestrictPtrTraits>()
        );
    }));

    return {rays_a, xyzs, dirs, deltas, ts, counter, rays_index};
}
