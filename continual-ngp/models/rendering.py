...
def render(model, rays_o, rays_d, conditions=None, **kwargs):
    """
    Render rays by
    ...
    results = render_func(model, rays_o, rays_d, hits_t, conditions, **kwargs)
...
@torch.no_grad()
def __render_rays_test(model, rays_o, rays_d, hits_t, conditions=None, **kwargs):
    """
    Render rays by
    ...
    conditions_c = torch.unsqueeze(torch.unsqueeze(conditions, dim=0), dim=0)
    ...
    while samples < kwargs.get('max_samples', MAX_SAMPLES):
        ...
        conditions_c = torch.tile(conditions, (xyzs[valid_mask].shape[0], 1))
        xyzs_c       = torch.cat([xyzs[valid_mask], conditions_c], dim=-1)
        sigmas[valid_mask], _rgbs = model(xyzs_c, dirs[valid_mask], conditions_c, **kwargs)
...
def __render_rays_train(model, rays_o, rays_d, hits_t, conditions=None, **kwargs):
    """
    Render rays by
    ...
    """
    '''
        In this method, RayMarch will give all the voxel that is being intersect
        by rays and for each rays there will be different number of voxel count.
        Each value in rays_a store ray_idx, start_idx and total_voxel for that ray.
        We can access any values in xyzs or dirs with ray_idx and from there total_voxel
        can be accessed.
        Here ws is weight/density of each sample point along the ray
    '''
    exp_step_factor = kwargs.get('exp_step_factor', 0.)
    results = {}
    ...
    # Here we have added conditional code
    conditions_c = torch.unsqueeze(torch.unsqueeze(conditions, dim=0), dim=0)
    conditions_c = torch.tile(conditions_c, (xyzs.shape[0], 1))
    ######################################
    xyzs_c = torch.cat([xyzs, conditions_c], dim=-1)
    sigmas, rgbs = model(xyzs_c, dirs, conditions_c, **kwargs)
