...
class NGPGUI:
    def __init__(self, hparams, K, img_wh, radius=2.5):
        ...
        self.mean_samples = 0
        self.img_mode = 0
        self.fade_range = torch.linspace(0, 1, 256).cuda()
        self.fade_idx   = 0
    ...
    def render_cam(self, cam):
        ...
        final_resutls = None
        total_class = [1, 2, 3, 4, 5, 6, 7, 8]
        for class_idx in total_class:
            conditions = torch.tensor(class_idx).cuda()
            results    = render(self.model, rays_o, rays_d, conditions,
                            **{'test_time': True,
