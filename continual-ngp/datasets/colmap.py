    ...
    def read_meta(self, split, hparams, **kwargs):
        ...
        for scene_idx, scene in enumerate(hparams.scene_lst, start=1):
            ...
            if hparams.continual:
                ...
                np.save('continual_poses_real/{}_pts3d.npy'.format(scene_idx), self.pts3d)
        ...
        if hparams.continual and split=='train':
            ...
            for scene_num in range(1, hparams.class_idx):
                ...
                # pts3d = np.load('continual_poses_real/{}_pts3d.npy'.format(scene_num))
                # pts_cam_dist = np.linalg.norm(pts3d[:, np.newaxis, :3] - poses[..., 3][np.newaxis, ...], axis=-1)
                # close_depth = np.percentile(pts_cam_dist, 0.1)
                # inf_depth = np.percentile(pts_cam_dist, 99.)
                # poses_copy = create_spiral_poses(poses, close_depth, inf_depth, N_views=hparams.previous_state)
                poses_copy = torch.FloatTensor(poses)
        ...
        if hparams.continual and split=='test' and not hparams.val_only:
            ...
            for scene_num in range(1, hparams.class_idx):
                ...
                close_depth = np.percentile(pts_cam_dist, 0.1)
                inf_depth = np.percentile(pts_cam_dist, 99.)
                poses_copy = create_spiral_poses(poses, close_depth, inf_depth, N_views=5)
